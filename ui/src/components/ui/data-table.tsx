          ...
          <div key={option.column} className="relative flex-1">
            {option.type === "select" ? (
              <Select
                ...
                onValueChange={(value) =>
                  handleSearchChange(option.column, value)
                }
              >
                <SelectTrigger className="h-10 w-full border-slate-200 bg-white">
                  <SelectValue
                    placeholder={
                      option.placeholder || `Select ${option.column}`
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="All Blood Types">All</SelectItem>
              ...
              <>
                ...
                <Input
                  placeholder={
                    option.placeholder || `Search by ${option.column}...`
                  }
                  value={searchValues[option.column] || ""}
                  onChange={(e) =>
                    handleSearchChange(option.column, e.target.value)
                  }
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="hover:bg-transparent">
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className={`text-${primaryColor}-700 font-medium h-11`}
                  >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {onPageSizeChange && pageSize && (
            ...
            >
              <SelectTrigger
                className={`h-9 w-[70px] border-slate-200 focus:ring-${primaryColor}-500/20`}
              >
