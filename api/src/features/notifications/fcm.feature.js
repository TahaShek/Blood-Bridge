...
const sendBloodRequestNotification = async ({
  bloodRequestId,
  bloodGroup,
  city,
  requestorName,
  ...
  try {
    const users = await User.find({
      _id: { $ne: requestorId },
      $or: [{ bloodGroup: bloodGroup }, { bloodGroup: "Any" }],
      "address.city": city,
    });
    ...
    const message = {
      notification: {
        title: title,
        body: `${requestorName} needs ${bloodGroup} near ${city} - urgency: ${urgencyLevel}  --${bloodRequestId}`,
        // data: {
...
const sendDonorAddedNotification = async (
  userId,
  bloodRequestId,
  donorId,
  donorName,
